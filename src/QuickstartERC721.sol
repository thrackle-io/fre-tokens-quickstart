...
/**
 * @title Example ERC721 QuickstartERC721
    ...
    ERC721Enumerable
{
    ...
    function setBaseURI(
        string memory _baseUri
    ) public virtual onlyRole(TOKEN_ADMIN_ROLE) {
        baseUri = _baseUri;
    }
     ...
     */
    function adminMint(address to) public onlyRole(TOKEN_ADMIN_ROLE) {
    ...
    function _beforeTokenTransfer(
        ...
        uint256 tokenId,
        uint256 batchSize
    ) internal override(ERC721, ERC721Enumerable) nonReentrant {
        // Rule Processor Module Check
        if (handlerAddress != address(0))
            require(
                IProtocolTokenHandler(handlerAddress).checkAllRules(
                    from == address(0) ? 0 : balanceOf(from),
                    to == address(0) ? 0 : balanceOf(to),
                    from,
                    to,
                    _msgSender(),
                    tokenId
